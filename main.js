...
let gameUnsubscribe = null; // Funktion zum Beenden des Firebase-Listeners
...
async function hostNewGame() {
    ...
    const gameData = {
        ...
        players: {
            [nickname]: { sips: 0, hand: [], receivedCards: [] }
        },
        displayJoined: false
    };
...
// --- ZENTRALER DATEN-LISTENER ---

function listenToGameUpdates(roomCode) {
    if (gameUnsubscribe) gameUnsubscribe(); // Alten Listener entfernen
    const gameRef = ref(db, `games/${roomCode}`);
